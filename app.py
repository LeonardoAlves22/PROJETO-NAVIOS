            ...
            with st.status("Sincronizando...", expanded=True) as status:
                ...
                if d_p[0]:
                    for eid in d_p[0].split()[-40:]:
                        ...
                        prospy.append({"subj": decodificar_cabecalho(m, "Subject"), "date": envio, "datas": extrair_datas_prospect(extrair_corpo_email(m), envio)})
                ...
                def processar(lista):
                    ...
                    for n in lista:
                        ...
                        final.append({"Navio": n, "Prospect Manhã": "✅" if any(e["date"].hour < 13 for e in match) else "❌", "Prospect Tarde": "✅" if any(e["date"].hour >= 13 for e in match) else "❌", "ETA": eta, "ETB": etb, "ETD": etd, "CLP": st_clp})
                    return final
