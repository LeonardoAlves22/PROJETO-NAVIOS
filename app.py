...
def salvar_banco(nome_id, eta, etb, etd, clp):
    try:
        ...
        # Mantém o dado do banco se o novo vier vazio
...
def extrair_datas_prospect(texto_puro, envio):
    ...
    # Regex para capturar ETA, ETB e ETD
    ...
    if st.button("🔄 ATUALIZAR AGORA", use_container_width=True, type="primary"):
        try:
            with st.status("🔍 Buscando e-mails de hoje...", expanded=True):
                ...
                data_imap = hoje_br.strftime("%d-%b-%Y") # Filtro IMAP por data (Hoje)
                ...
                if d_p[0]:
                    for eid in d_p[0].split():
                        ...
                        # Filtro rigoroso: Apenas e-mails de hoje (00:00 até agora)
...
with c2:
    if st.button("📧 ENVIAR POR E-MAIL", use_container_width=True):
        ...
        else: st.warning("Atualize os dados antes de enviar.")
