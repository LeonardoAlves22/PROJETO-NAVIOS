...
# --- APOIO ---
def decodificar_texto_otimizado(payload):
    if not payload: return ""
    txt = payload.decode(errors='ignore')
    # Trata as tags <br> e <div> transformando em quebras de linha antes de limpar
...
def extrair_datas_prospect(texto_puro, envio):
    ...
    def parse_data(s):
        ...
        if m_mes and m_dia:
            ...
            dt_en = datetime(envio.year, mes, dia, tzinfo=BR_TZ)
            if dt_en < (hoje - timedelta(days=15)): return "-"
    ...
    if st.button("🔄 ATUALIZAR AGORA", use_container_width=True, type="primary"):
        try:
            with st.status("🚀 Sincronizando dados...", expanded=True) as status:
                ...
                slz_raw, bel_raw = [], []
                if d_l[0]:
                    _, d = mail.fetch(d_l[0].split()[-1], '(BODY.PEEK[])') # Busca completa para evitar erro de tag
                    msg_lista = email.message_from_bytes(d[0][1])
                    
                    corpo_bruto = ""
                    if msg_lista.is_multipart():
                        for p in msg_lista.walk():
                            if p.get_content_type() in ['text/plain', 'text/html']:
                                corpo_bruto += p.get_payload(decode=True).decode(errors='ignore')
                    else:
                        corpo_bruto = msg_lista.get_payload(decode=True).decode(errors='ignore')

                    # Limpa o HTML e separa por linhas
                    corpo_limpo = decodificar_texto_otimizado(corpo_bruto.encode())
                    linhas = [l.strip() for l in corpo_limpo.split('\n') if len(l.strip()) > 2]
                    ...
                    for linha in linhas:
                        ...
                        if secao == "SLZ": slz_raw.append(linha)
                        elif secao == "BEL": bel_raw.append(linha)
                ...
                def processar(lista, belem=False):
                    ...
                    for n in lista:
                        ...
                        res.append({"Navio": n_id if belem else n, 
                                    "Prospect Manhã": "✅" if any(e["date"].hour < 13 and e["date"].date() == agora.date() for e in matches) else "❌", 
                                    "Prospect Tarde": "✅" if any(e["date"].hour >= 13 and e["date"].date() == agora.date() for e in matches) else "❌", 
