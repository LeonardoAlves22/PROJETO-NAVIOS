...
def executar_busca():
    try:
        ...
        mail.login("leonardo.alves@wilsonsons.com.br", "SUA_SENHA_AQUI")
        ...
        for e_id in ids[0].split():
            ...
            subj = "".join(
                str(c.decode(ch or 'utf-8', errors='ignore') if isinstance(c, bytes) else c)
        ...
        rel_m = "📋 STATUS MANHÃ (GERAL)\n"
        rel_t = "🕒 STATUS TARDE (PÓS-14H)\n"
        ...
        for titulo, lista, rems in [("SÃO LUÍS", slz_l, REM_SLZ), ("BELÉM", bel_l, REM_BEL)]:
            ...
            for n in lista:
                m_g = [
                    em for em in db
                    if n in em["subj"]
                    and any(r in em["from"] for r in rems)
